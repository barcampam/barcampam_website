defmodule BarcampWeb.TalkControllerTest do
  ...
  @create_attrs %{description: "some description", room: "some room", time_from: ~N[2010-04-17 14:00:00], time_to: ~N[2010-04-17 14:00:00], title: "some title"}
  ...
  describe "index" do
    test "lists all talks", %{conn: conn} do
      ...
      assert html_response(conn, 200) =~ "Talks"
    end
  end
  ...
  describe "create talk" do
    test "redirects to show when data is valid", %{conn: conn} do
      ...
      assert html_response(conn, 200) =~ "Talk Details"
    end
