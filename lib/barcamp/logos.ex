defmodule Barcamp.Logos do
  ...
  def paginate_sponsors(params \\ %{}) do
    ...
    with {:ok, filter} <-
           Filtrex.parse_params(filter_config(:sponsors), params["sponsor"] || %{}),
  ...
  def paginate_generals(params \\ %{}) do
    ...
    with {:ok, filter} <-
           Filtrex.parse_params(filter_config(:generals), params["general"] || %{}),
  ...
  def paginate_partners(params \\ %{}) do
    ...
    with {:ok, filter} <-
           Filtrex.parse_params(filter_config(:partners), params["partner"] || %{}),
  ...
  """
  # def create_partner(%{"logo" => %Plug.Upload{content_type: content_type, path: path}} = attrs) do
  #   base = path |> File.read!() |> Base.encode64()
  #   logo = "#{content_type};base64,#{base}"
  #   attrs = Map.put(attrs, "logo", logo)
